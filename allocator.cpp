...
namespace ghoard {
    ...
    allocator::heap_holder_cls::heap_holder_cls() {
        heaps = (heap*) raw_allocate(HEAP_SIZE * (1 + HEAP_CNT));
        for (int i = 0; i < HEAP_CNT; ++i) ((heap*) get_heap(i))->init();
    }

    heap* allocator::heap_holder_cls::get_heap(int i) {
        return (heap*) (((char*) heaps) + HEAP_SIZE * i);

    }

    void * allocator::allocate(size_t data_size, size_t alignment) {
    ...
    size_t allocator::get_acceptable_block_size(size_t data_size, size_t meta_size, size_t alignment) {
        return data_size + meta_size + alignment - 1;
    }

    void * allocator::allocate_large_block(size_t data_size, size_t alignment = DEFAULT_ALIGNMENT) {
    ...
    heap * allocator::get_global_heap() {
        return heap_holder.get_heap(0);
    }

    heap * allocator::get_current_heap() {
